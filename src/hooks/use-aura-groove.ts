...
export const useAuraGroove = () => {
...
}, [drumSettings.volume, toast, requestNewScoreFromWorker]);
  ...
  const handleTogglePlay = useCallback(async () => {
    ...
    const Tone = toneRef.current;
    if (!Tone || !Tone.Transport || !isInitializedRef.current) {
      toast({ variant: "destructive", title: "Audio Engine Failed", description: "Could not initialize audio components. Please refresh the page."});
      return;
    }
  ...
  useEffect(() => {
    if (musicWorkerRef.current && isInitializedRef.current) {
  ...
  useEffect(() => {
    const Tone = toneRef.current;
    if (isInitializedRef.current && Tone) {
    ...
    return () => {
        if (scoreRequestLoopIdRef.current !== null && toneRef.current) {
            toneRef.current.Transport.clear(scoreRequestLoopId_Ref.current);
        }
    };
  }, [bpm, isPlaying, requestNewScoreFromWorker]);
