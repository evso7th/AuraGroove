...
export class SoloSynthManager {
    ...
    public triggerAttackRelease(notes: string | string[], duration: Tone.Unit.Time, time?: Tone.Unit.Time, velocity?: number) {
        console.log(`[SOLO_SYNTH_TRACE] FINAL_LINK_CHECK. Instrument: ${this.currentInstrument}. Notes: ${JSON.stringify(notes)}, Duration: ${duration}, Time: ${time}, Velocity: ${velocity}`);
        ...
        notesToPlay.forEach(note => {
            ...
            console.log(`[SOLO_SYNTH_TRACE] Voice ${this.nextVoiceIndex} volume: ${voice.volume.value}`);
